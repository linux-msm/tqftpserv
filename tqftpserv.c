...
/**
 * handle_unlink() - handle a WRQ carrying the "unlink" option
 * @filename: path of the file to remove
 * @sq: remote address that issued the request
 *
 * Instead of opening @filename for writing, remove it and report the
 * outcome back to the client with an OACK (success) or ERROR packet.
 * Ported from tftp_thread_unlink_handler() in tftp_server.c.
 */
